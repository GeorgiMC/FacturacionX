...
namespace CapaLogica
{
    ...
    {
        public List<CompraDatos> SeleccionarCompras(string cia)
        {
            List<CompraDatos> lista = new List<CompraDatos>();
            DataSet ds = CompraDatos.SeleccionarTodos(cia);

            foreach (DataRow row in ds.Tables[0].Rows)
            {
                CompraDatos obj = new CompraDatos();
